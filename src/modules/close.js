...
module.exports = bot => {
  ...
  async function applyScheduledCloses() {
    ...
    for (const thread of threadsToBeClosed) {
      ...
      utils.postLog(utils.trimAll(`
        Тикет с ${thread.user_name} (${thread.user_id}) Был закрыт ${thread.scheduled_close_name}
        Logs: ${logUrl}
      `));
    }
  }
  ...
  bot.registerCommand('close', async (msg, args) => {
    ...
    if (msg.channel instanceof Eris.PrivateChannel) {
      ...
      await messageQueue.add(async () => {
        thread.postSystemMessage('Тикет был закрыт');
    ...
    const logUrl = await thread.getLogUrl();
    utils.postLog(utils.trimAll(`
      Тикет с ${thread.user_name} (${thread.user_id}) Был закрыт  ${closedBy}
      Logs: ${logUrl}
    `));
  });
  ...
  bot.on('channelDelete', async (channel) => {
    ...
    console.log(`[INFO] Автоматическое закрытие тикета с ${thread.user_name} потому что канал был удален`);
    if (config.closeMessage) await thread.postToUser(config.closeMessage).catch(() => {});
    await thread.close(true);

    const logUrl = await thread.getLogUrl();
    utils.postLog(utils.trimAll(`
      Автоматическое закрытие тикета с ${thread.user_name} (${thread.user_id}) потому что канал был удален
      Logs: ${logUrl}
    `));
  });
};
