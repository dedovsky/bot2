  ...
  async function applyScheduledSuspensions() {
    ...
    for (const thread of threadsToBeSuspended) {
      if (thread.status === THREAD_STATUS.OPEN) {
        ...
        await thread.postSystemMessage(`Заморуживаю ${thread.scheduled_suspend_name}. Если вы хотите разморозить, пишите !unsuspend\`!unsuspend\``);
      }
    }
  }
    ...
    if (args.length) {
      ...
      if (args.includes('cancel') || args.includes('c')) {
        ...
        if (thread.scheduled_suspend_at) {
          ...
          thread.postSystemMessage(`Hfpvjhf;bdf.`);
        }
      ...
      if (delayStringArg) {
        ...
        if (delay === 0 || delay === null) {
          thread.postSystemMessage(`НЕВЕРНЫЙ ФОРМАТ! Пример верного"1h30m"`);
          return;
        }
        ...
        thread.postSystemMessage(`Замораживаю на ${utils.humanizeDelay(delay)}.`);
